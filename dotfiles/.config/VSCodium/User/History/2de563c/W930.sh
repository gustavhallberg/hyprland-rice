...
WOFI_CONF="$HOME/.config/wofi-picker/wallpaper.conf"
...
list_wallpapers() {
    ...
    for i in "$DIR"/*.{jpg,jpeg,png,webp}; do
        if [ -f "$i" ]; then
            filename=$(basename "$i")
            # Create larger thumbnails for the grid (600x600)
...
# 4. EXECUTE WOFI
SELECTED=$(list_wallpapers | wofi --conf "$WOFI_CONF" --search /dev/null --cache-file /dev/null)
...
if [ -f "$FULL_PATH" ]; then
    ...
    notify-send "Error" "Selection failed to resolve to a file."
fi